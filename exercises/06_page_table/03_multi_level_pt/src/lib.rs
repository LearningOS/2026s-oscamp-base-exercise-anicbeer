...
impl Sv39PageTable {
    ...
    pub fn map_page(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        // Level 2: 检查或创建 L1 页表
        let l2_pte = self.nodes.get(&self.root_ppn).unwrap().entries[vpn2];
        let l1_ppn = if (l2_pte & PTE_V) == 0 {
            let new_ppn = self.alloc_node();
            self.nodes.get_mut(&self.root_ppn).unwrap().entries[vpn2] = (new_ppn << PPN_SHIFT) | PTE_V;
            new_ppn
        } else {
            l2_pte >> PPN_SHIFT
        };
        ...
        // Level 1: 检查或创建 L0 页表
        let l1_pte = self.nodes.get(&l1_ppn).unwrap().entries[vpn1];
        let l0_ppn = if (l1_pte & PTE_V) == 0 {
            let new_ppn = self.alloc_node();
            self.nodes.get_mut(&l1_ppn).unwrap().entries[vpn1] = (new_ppn << PPN_SHIFT) | PTE_V;
            new_ppn
        } else {
            l1_pte >> PPN_SHIFT
        };
        ...
        // Level 0: 写入叶子 PTE
        let ppn = pa >> 12;
        self.nodes.get_mut(&l0_ppn).unwrap().entries[vpn0] = (ppn << PPN_SHIFT) | flags;
    }
    ...
    pub fn translate(&self, va: u64) -> TranslateResult {
        ...
        // Level 2
        let root_node = self.nodes.get(&self.root_ppn).unwrap();
        let l2_pte = root_node.entries[vpn2];
        if (l2_pte & PTE_V) == 0 {
        ...
        if (l2_pte & (PTE_R | PTE_W | PTE_X)) != 0 {
            let ppn = l2_pte >> PPN_SHIFT;
            return TranslateResult::Ok((ppn << 12) | offset);
        }

        // Level 1
        let l1_ppn = l2_pte >> PPN_SHIFT;
        let l1_node = self.nodes.get(&l1_ppn).unwrap();
        ...
        if (l1_pte & (PTE_R | PTE_W | PTE_X)) != 0 {
            let ppn = l1_pte >> PPN_SHIFT;
            // 大页的 offset 包含低 21 位
            let large_offset = va & ((1 << 21) - 1);
            return TranslateResult::Ok((ppn << 12) | large_offset);
        }

        // Level 0
        let l0_ppn = l1_pte >> PPN_SHIFT;
        let l0_node = self.nodes.get(&l0_ppn).unwrap();
    ...
    pub fn map_superpage(&mut self, va: u64, pa: u64, flags: u64) {
        ...
        // Level 1: 写入叶子 PTE
        let ppn = pa >> 12;
        self.nodes.get_mut(&l1_ppn).unwrap().entries[vpn1] = (ppn << PPN_SHIFT) | flags;
    }
}
